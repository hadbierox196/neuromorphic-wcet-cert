...
class SeizureLIFNet(nn.Module):
    ...
    def forward(self, x_flat: torch.Tensor) -> torch.Tensor:
        ...
        return spk3  # (batch, 2) spike output for this step

    def forward_sequence(self, x_flat: torch.Tensor, n_steps: int = 512) -> torch.Tensor:
        """Replay n_steps to get an output spike-count vector per §2.2.3 loss."""
        spikes = [self.forward(x_flat) for _ in range(n_steps)]
